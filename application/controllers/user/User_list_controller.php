...
class User_list_controller extends CI_Controller {
	...
	public function index()
	{
		...
		if(html_escape($this->input->post('keyword')) == NULL)
		{
          ...
          $data['user_list'] = $this->register_user_model->get_users($limit_per_page,$start);
		  $datakey = array(
		  'key'  => 'show',
		  );
        ...
        else 
		{
          ...
          $data['user_list'] = $this->register_user_model->search_tag($keys);
		}

        $data['main_view'] = 'user/user_list';
	...
	{
	    $query2 = $this->register_user_model->get_total_user_rows();

	    //Pagination library
	    $this->load->library('pagination');

	    //Codeigniter Pagination
	    $config['base_url'] = base_url('user/user_list_controller/index');
