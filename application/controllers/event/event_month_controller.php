...
class Event_month_controller extends CI_Controller {
	...
	public function index()
	{
		...
		if(html_escape($this->input->post('keyword')) == NULL)
		{
          ...
          $data['events_list'] = $this->event_model->find_event_per_month($limit_per_page,$start);
		  $datakey = array(
		  'key'  => 'show',
		  );
        ...
        else 
		{
          ...
          $data['events_list'] = $this->event_model->event_month_search_tag($keys);
		}

        $data['main_view'] = 'event/event_this_month';
	...
	{
	    $query2 = $this->event_model->events_per_month();

	    //Pagination library
	    $this->load->library('pagination');

	    //Codeigniter Pagination
	    $config['base_url'] = base_url('event/event_month_controller/index');
